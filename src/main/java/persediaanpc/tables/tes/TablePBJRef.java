...
public class TablePBJRef implements JMTableInterface{
    private final String title=R.label("TITLE_PJ_PBJ");
    ...
    public static TablePBJRef create(String query,FormTableLookup parent){
        return new TablePBJRef(query,null,parent);
    }
    
    public static TablePBJRef create(String query,FormTable parent){
        return new TablePBJRef(query,parent,null);
    }
    
    public TablePBJRef(String query,FormTable parentTable,FormTableLookup parentTableLookup){
        ...
        if(parentTableLookup!=null){
            ...
            this.parentTableLookup.setFilterAction(new Runnable() {
                @Override
                public void run() {
                    TablePBJRef.this.dbObject.filter(TablePBJRef.this.parentTableLookup.getSearch().getText());
                }
            });
        }
        ...
        if(parentTable!=null){
            ...
            this.parentTable.setFilterAction(new Runnable() {
                @Override
                public void run() {
                    TablePBJRef.this.dbObject.filter(TablePBJRef.this.parentTable.getSearch().getText());
                }
            });
        }
        ...
        this.dbObject.getStyle()
                .setLabel(0,R.label("ID_JAB_PBJ"))
                .setLabel(1,R.label("SINGKAT_JAB_PBJ"))
                .setLabel(2,R.label("NM_JAB_PBJ"))
                .setColHidden(0);
        ...
        this.dbObject.setName("p_ref_jab_pbj");
    ...
    public JMRow select(){
        ...
        okCancelRunnables.add(new Runnable() {
            @Override
            public void run() {
                TablePBJRef.this.selectedRow=TablePBJRef.this.dbObject.getCurrentRow();
            }
        });
        okCancelRunnables.add(new Runnable() {
            @Override
            public void run() {
                TablePBJRef.this.selectedRow=null;
            }
        });
    ...
    private void openInput(boolean editing, boolean adding){
        ...
        if(this.parentTable!=null)InputItem.create(TablePBJRef.this.dbObject,this.parentTable,editing,adding);
        if(this.parentTableLookup!=null)InputItem.create(TablePBJRef.this.dbObject,this.parentTableLookup,editing,adding);
    }
    ...
    private void addListener(){
        this.table.addKeyListener(new KeyListener(){
            ...
            public void keyReleased(KeyEvent e) {
                if(e.getKeyCode()==e.VK_ENTER){
                    if(TablePBJRef.this.parentTableLookup!=null){
                        TablePBJRef.this.selectThis();
                    }else TablePBJRef.this.openInput(false,false);
                }
            }
        });
        ...
        this.btnGroup.getBtnAdd().addAction(new Runnable(){
            @Override
            public void run() {
                TablePBJRef.this.openInput(true,true);
            }
        });
        this.btnGroup.getBtnEdit().addAction(new Runnable(){
            @Override
            public void run() {
                TablePBJRef.this.openInput(true,false);
            }
        });
        this.btnGroup.getBtnView().addAction(new Runnable(){
            @Override
            public void run() {
                TablePBJRef.this.openInput(false,false);
            }
        });
    }
    ...
    public Runnable filter(JMPCInputStringTFWeblaf textField){
        return new Runnable(){
            @Override
            public void run() {
                TablePBJRef.this.dbObject.filter(textField.getText());
            }
        };
    }
