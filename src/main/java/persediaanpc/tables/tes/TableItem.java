...
public class TableItem implements JMTableInterface{
    ...
    public TableItem(String query,FormTable parentTable,FormTableLookup parentTableLookup){
        ...
        this.dbObject.getStyle()
                .setLabel(0,R.label("ID_ITEM"))
                .setLabel(1,R.label("KET_KAT"))
                .setLabel(2,R.label("NM_ITEM"))
                .setLabel(3,R.label("TOTAL_STOCK"))
                .setLabel(4,R.label("BIDANG_STOCK"))
                .setLabel(5,R.label("SATUAN"))
                .setLabel(6,R.label("ID_KAT"))
                .setColHidden(0)
                .setColHidden(6);
    ...
    public JMRow select(){
        ...
        okCancelRunnables.add(new Runnable() {
            @Override
            public void run() {
                TableItem.this.selectedRow=TableItem.this.dbObject.getCurrentRow();
            }
        });
        okCancelRunnables.add(new Runnable() {
            @Override
    ...
    private void addListener(){
        ...
        this.btnGroup.getBtnAdd().addAction(new Runnable(){
            @Override
            public void run() {
                TableItem.this.openInput(true,true);
            }
        });
        this.btnGroup.getBtnEdit().addAction(new Runnable(){
            @Override
            public void run() {
                TableItem.this.openInput(true,false);
            }
        });
        this.btnGroup.getBtnView().addAction(new Runnable(){
            @Override
            public void run() {
                TableItem.this.openInput(false,false);
            }
        });
    }
    ...
    public Runnable filter(JMPCInputStringTFWeblaf textField){
        return new Runnable(){
            @Override
            public void run() {
                TableItem.this.dbObject.filter(textField.getText());
            }
        };
    }
