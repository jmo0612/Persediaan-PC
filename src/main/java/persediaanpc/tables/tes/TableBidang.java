...
public class TableBidang implements JMTableInterface{
    ...
    public static TableBidang create(String query,FormTableLookup parent){
        return new TableBidang(query,null,parent);
    }
    
    public static TableBidang create(String query,FormTable parent){
        return new TableBidang(query,parent,null);
    }
    
    public TableBidang(String query,FormTable parentTable,FormTableLookup parentTableLookup){
        ...
        if(parentTableLookup!=null){
            ...
            this.parentTableLookup.setFilterAction(new Runnable() {
                @Override
                public void run() {
                    TableBidang.this.dbObject.filter(TableBidang.this.parentTableLookup.getSearch().getText());
                }
            });
        }
        ...
        if(parentTable!=null){
            ...
            this.parentTable.setFilterAction(new Runnable() {
                @Override
                public void run() {
                    TableBidang.this.dbObject.filter(TableBidang.this.parentTable.getSearch().getText());
                }
            });
        }
        ...
        this.dbObject.getStyle()
                .setLabel(0,R.label("ID_BIDANG"))
                .setLabel(1,R.label("NM_BIDANG"))
                .setColHidden(0);
        ...
        this.dbObject.setName("p_tb_bidang");
    ...
    public JMRow select(){
        ...
        okCancelRunnables.add(new Runnable() {
            @Override
            public void run() {
                TableBidang.this.selectedRow=TableBidang.this.dbObject.getCurrentRow();
            }
        });
        okCancelRunnables.add(new Runnable() {
            @Override
            public void run() {
                TableBidang.this.selectedRow=null;
            }
        });
    ...
    private void openInput(boolean editing, boolean adding){
        ...
        if(this.parentTableLookup!=null)InputBidang.create(TableBidang.this.dbObject,this.parentTableLookup,editing,adding);
    }
    ...
    private void addListener(){
        this.table.addKeyListener(new KeyListener(){
            ...
            public void keyReleased(KeyEvent e) {
                if(e.getKeyCode()==e.VK_ENTER){
                    if(TableBidang.this.parentTableLookup!=null){
                        TableBidang.this.selectThis();
                    }else TableBidang.this.openInput(false,false);
                }
            }
        });
        ...
        this.btnGroup.getBtnAdd().addAction(new Runnable(){
            @Override
            public void run() {
                TableBidang.this.openInput(true,true);
            }
        });
        this.btnGroup.getBtnEdit().addAction(new Runnable(){
            @Override
            public void run() {
                TableBidang.this.openInput(true,false);
            }
        });
        this.btnGroup.getBtnView().addAction(new Runnable(){
            @Override
            public void run() {
                TableBidang.this.openInput(false,false);
            }
        });
    }
    ...
    public Runnable filter(JMPCInputStringTFWeblaf textField){
        return new Runnable(){
            @Override
            public void run() {
                TableBidang.this.dbObject.filter(textField.getText());
            }
        };
    }
